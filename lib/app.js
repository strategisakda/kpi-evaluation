...
// ---------------------------------------------------------------------
// DELETE /api/admin/users/:username — ลบบัญชีผู้ใช้ (SuperAdmin เท่านั้น)
// กันลบตัวเอง, กันลบ SuperAdmin คนสุดท้าย, และยกเลิกการมอบหมายตรวจ KPI
// ที่บัญชีนี้รับผิดชอบอยู่ก่อน (กัน FK constraint กับ kpi_master)
// ---------------------------------------------------------------------
app.delete('/api/admin/users/:username', requireAuth(['SuperAdmin']), async (req, res) => {
  try {
    ...
    if (!target) return res.status(404).json({ ok: false, error: 'ไม่พบผู้ใช้งานนี้' });
    ...
    if (target.role === 'SuperAdmin') {
      const { count } = await supabase
        ...
        .eq('role', 'SuperAdmin');
      if ((count || 0) <= 1) {
        return res.status(400).json({ ok: false, error: 'ต้องมีบัญชีผู้ดูแลระบบ (SuperAdmin) อย่างน้อย 1 บัญชีเสมอ' });
      }
    }

    await supabase.from('kpi_master').update({ assigned_reviewer_username: null }).eq('assigned_reviewer_username', username);
...
app.post('/api/evidence/confirm-upload', requireAuth(['User']), async (req, res) => {
  try {
    ...
    // แจ้งเตือนอีเมลผู้ตรวจที่ถูกมอบหมาย KPI นี้ ว่ามีหลักฐานใหม่รอตรวจ
    // (await ก่อนตอบกลับ เพราะ Vercel serverless function อาจ freeze ทันทีหลังส่ง response)
    const { data: kpi } = await supabase
      .from('kpi_master')
      .select('kpi_name, assigned_reviewer_username')
      .eq('kpi_id', kpiId)
      .maybeSingle();
    if (kpi && kpi.assigned_reviewer_username) {
      const { data: reviewer } = await supabase
        .from('users')
        .select('contact_email')
        .eq('username', kpi.assigned_reviewer_username)
        .maybeSingle();
      if (reviewer && reviewer.contact_email) {
        await sendNotificationEmail(
          reviewer.contact_email,
          `[KPI] มีหลักฐานใหม่รอตรวจ: ${kpi.kpi_name}`,
          `อำเภอ${district} ได้ส่งหลักฐานสำหรับตัวชี้วัด "${kpi.kpi_name}" (${kpiId}) เรียบร้อยแล้ว กรุณาเข้าสู่ระบบเพื่อตรวจให้คะแนน`
        );
      }
    }
...
app.post('/api/evaluation/score', requireAuth(['Admin', 'SuperAdmin']), async (req, res) => {
  try {
    ...
    // แจ้งเตือนอีเมลผู้ใช้ประจำอำเภอนี้ ว่าตัวชี้วัดได้รับการตรวจ/ให้คะแนนแล้ว
    const { data: kpi } = await supabase.from('kpi_master').select('kpi_name').eq('kpi_id', kpiId).maybeSingle();
    const { data: districtUsers } = await supabase
      ...
      .eq('role', 'User')
      .limit(1);
    const districtUser = districtUsers && districtUsers[0];
    if (districtUser && districtUser.contact_email) {
      await sendNotificationEmail(
        districtUser.contact_email,
        `[KPI] ตัวชี้วัด "${kpi ? kpi.kpi_name : kpiId}" ได้รับการตรวจแล้ว`,
        `ตัวชี้วัด "${kpi ? kpi.kpi_name : kpiId}" (${kpiId}) ของอำเภอ${district} ได้รับคะแนน ${scoreNum} จาก ${req.user.username} เรียบร้อยแล้ว`
      );
    }
