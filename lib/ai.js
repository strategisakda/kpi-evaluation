...
async function analyzeEvidenceWithAI(supabase, bucket, { district, kpiName, kpiNote, kpiLevels, evidenceFilePath, evidenceMimeType }) {
  ...
  const promptText =
    ...
    'ข้อสำคัญข้อที่ 3 (หลักฐานเชิงตัวเลข): หากเกณฑ์ระดับคะแนนของตัวชี้วัดนี้กำหนดเป็นตัวเลข/ร้อยละ/จำนวน (เช่น "ร้อยละการเบิกจ่าย", "ไม่น้อยกว่าร้อยละ X") ' +
    'เอกสารหลักฐานต้องแสดง "ตัวเลขผลการดำเนินงานจริง" ที่วัดตรงกับตัวชี้วัดนั้นโดยตรง (เช่น ยอดเบิกจ่ายจริงเทียบยอดจัดสรร, จำนวนที่ทำได้จริงเทียบเป้าหมาย) ' +
    'เอกสารประเภทแผนงาน/แผนยุทธศาสตร์/แผนพัฒนาที่เพียงกล่าวถึงงบประมาณหรือเป้าหมายในเชิงกว้าง ๆ โดยไม่มีตัวเลขผลการดำเนินงานจริงที่ตรงประเด็น ' +
    'ถือว่า "ไม่สอดคล้อง" หรือ "ไม่แน่ใจ" (แล้วแต่กรณี) เสมอ ห้ามตัดสินว่า "สอดคล้อง" เพียงเพราะเอกสารมีคำว่างบประมาณ/เป้าหมายปรากฏอยู่โดยไม่มีตัวเลขผลจริงรองรับ\n' +
    'โดยไม่ต้องตัดสินว่าควรได้กี่คะแนน (การตัดสินคะแนนเป็นหน้าที่ของนักวิชาการผู้ตรวจเท่านั้น) ให้ตอบ 5 เรื่อง:\n' +
    '1) เนื้อหาในไฟล์นี้สอดคล้องกับตัวชี้วัดและเกณฑ์ที่กำหนดไว้หรือไม่ ทั้งในแง่ระดับ/ขอบเขตของเอกสาร (รายบุคคล vs ชุมชน/พื้นที่), เป็นเอกสารของอำเภอ "' + district + '" จริงหรือไม่, และมีตัวเลขผลการดำเนินงานจริงรองรับหรือไม่ (ถ้าตัวชี้วัดต้องการตัวเลข)\n' +
    ...
    '5) เทียบเนื้อหาในเอกสารกับ "เกณฑ์ระดับคะแนน 1-5" ที่ให้ไว้ข้างต้นทีละระดับ แล้วประเมินว่าเอกสารนี้แสดงหลักฐานเข้าข่ายได้ถึงระดับใด (achievedLevel เป็นตัวเลข 0-5 โดย 0 หมายถึงไม่เข้าข่ายแม้แต่ระดับ 1) ' +
    'พร้อมอธิบายเหตุผลสั้นๆ ว่าทำไมถึงประเมินระดับนี้ อ้างอิงจากเนื้อหาจริงในเอกสาร (levelReasoning) — ข้อมูลนี้เป็นเพียงข้อมูลประกอบการพิจารณาเบื้องต้นให้นักวิชาการเท่านั้น ' +
    'ไม่ใช่การตัดสินคะแนนจริง นักวิชาการยังคงเป็นผู้ตัดสินใจขั้นสุดท้ายเสมอ';
      ...
      format: {
        ...
        schema: {
          ...
          properties: {
            ...
            achievedLevel: { type: 'integer', enum: [0, 1, 2, 3, 4, 5] },
            levelReasoning: { type: 'string' },
          },
          required: ['consistency', 'summary', 'observations', 'recommendations', 'achievedLevel', 'levelReasoning'],
...
// แชทบอทถามตอบเรื่องตัวชี้วัด — อ้างอิงจากข้อมูล KPI Master ทั้งหมดเท่านั้น (ไม่เดาเอง)
async function answerKpiQuestion(kpiList, question) {
  const apiKey = requireApiKey();

  const kpiText = kpiList
    .map((k) => {
      const levelsText = k.levels.map((lv, i) => `    ระดับ ${i + 1}: ${lv || '-'}`).join('\n');
      const reviewerText = k.reviewerName
        ? `${k.reviewerName}${k.reviewerContactEmail ? ' (' + k.reviewerContactEmail + ')' : ''}`
        : 'ยังไม่มอบหมาย';
      return (
        `[${k.kpiId}] ${k.name} (${k.category === 'TEAM' ? 'ตัวชี้วัดทีม' : 'ตัวชี้วัดรายบุคคล'}, น้ำหนัก ${k.weight}%)\n` +
        `  รายละเอียด/เกณฑ์: ${k.note || '-'}\n` +
        levelsText + '\n' +
        `  แหล่งข้อมูล: ${k.dataSource || '-'}\n` +
        `  ผู้ประสานงานข้อมูล: ${k.contact || '-'}\n` +
        `  นักวิชาการผู้ตรวจ: ${reviewerText}`
      );
    })
    .join('\n\n');

  const promptText =
    'ข้อมูลตัวชี้วัด (KPI) ทั้งหมดของระบบตรวจคำรับรองการปฏิบัติราชการ สำนักงานพัฒนาชุมชนจังหวัดพัทลุง:\n\n' +
    kpiText +
    '\n\nคำถามจากผู้ใช้งาน: "' + question + '"\n\n' +
    'ตอบคำถามนี้เป็นภาษาไทย โดยอ้างอิงจากข้อมูลตัวชี้วัดที่ให้ไว้ข้างต้นเท่านั้น ห้ามเดาหรือแต่งข้อมูลที่ไม่มีในรายการ ' +
    'ถ้าคำถามเกี่ยวกับตัวชี้วัดที่ไม่มีในรายการ หรือถามเรื่องที่ไม่มีข้อมูลรองรับ ให้ตอบตรงๆ ว่าไม่มีข้อมูลนี้ในระบบ ' +
    'ตอบให้กระชับ อ่านง่าย ตรงประเด็น และระบุรหัสตัวชี้วัด (เช่น T1, P3) ที่เกี่ยวข้องกับคำตอบด้วย';
    ...
    output_config: {
      format: {
        ...
        schema: {
          ...
          properties: {
            answer: { type: 'string' },
            relevantKpiIds: { type: 'array', items: { type: 'string' } },
          },
          required: ['answer', 'relevantKpiIds'],
          additionalProperties: false,
        },
      },
    },
  ...
  return {
    ...
    relevantKpiIds: result.relevantKpiIds || [],
  };
}

module.exports = { analyzeEvidenceWithAI, generateProvinceNarrative, answerKpiQuestion };
